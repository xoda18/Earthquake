...
def notify_orchestrator(data: dict):
    """Notify orchestrator that an earthquake was detected, triggering the pipeline."""
    if not ORCHESTRATOR_URL:
        return
    try:
        requests.post(f"{ORCHESTRATOR_URL}/step/done", json={
            "step": "earthquake_detected",
            "status": "success",
            "detail": f"prob={data.get('probability', 0):.3f} amp={data.get('amplitude_g', 0):.4f}g",
        }, timeout=3)
    except Exception:
        pass
        ...
        while True:
            ...
            if len(buf) >= WINDOW and (now - last_classify) >= classify_interval:
                ...
                if use_blackboard and (now - last_blackboard) >= 1.0:
                    ...
                    threading.Thread(target=post_to_blackboard, args=(bb_data,), daemon=True).start()

                    # Notify orchestrator to trigger pipeline on earthquake
                    if is_eq:

...
Two **AprilTags** (family `tag25h9`, IDs 0 and 1) are physically mounted at
...
├── input/
│   ├── before-DD-MM-YY.tif/.jpg  ← reference photo (the "ground truth" state)
│   └── after-DD-MM-YY.tif/.jpg   ← comparison photo (taken later / from different height)
├── output/
│   └── diff-DD-MM-YY_<time_generated>.png  ← after image with change heatmap overlaid
├── detect.py
├── requirements.txt
├── Dockerfile
└── docker-compose.yml
...
Claude Code must create **three files** alongside `detect.py`:
...
`input/` and `output/` are mounted as volumes — the container never
touches anything outside those two folders. Drop images in, run, get results.
...
### 1. Load images
- Scan `input/` for files matching `before-*` and `after-*` with any extension
  in `INPUT_EXTENSIONS` (`.tif`, `.tiff`, `.jpg`, `.jpeg`, `.JPG`, `.JPEG`).
- Pair them by date suffix — `before-DD-MM-YY` pairs with `after-DD-MM-YY`.
- Read each pair using `cv2.imread(..., cv2.IMREAD_COLOR)`.
...
- Use `pupil_apriltags.Detector(families='tag25h9')`.
...
- Match tags **by ID** across the two images. Do not assume any fixed spatial
  position — tag 0 may be anywhere (top-left, bottom-right, etc.) and its
  position is guaranteed to be the same physical corner in both photos.
- Because positions are consistent across both images, simply pair each tag ID
  found in `after` with the same tag ID found in `before`.
...
- Compute `H, mask = cv2.findHomography(pts_after, pts_before, cv2.RANSAC, 5.0)`.
  ...
  - `pts_before`: 8×2 array of the **same** tag corners (matched by ID) in the before image.
...
### 9. Save output
- `cv2.imwrite('output/diff-DD-MM-YY_<time_generated>.png', output_bgr)`
- Also save `output/diff_debug-DD-MM-YY_<time_generated>.png` — a 3-panel image
  (before crop | after crop | heatmap only) for debugging alignment. Use `np.hstack` or `matplotlib`.
...
OUTPUT_DIFF      = 'output/diff'        # timestamp appended at runtime
OUTPUT_DEBUG     = 'output/diff_debug'  # timestamp appended at runtime
```
...
| No matching before/after pair found | Print clear message, exit code 1 |
...
```
┌─────────────────────────┐
│  [TAG ID=?]             │  ← either tag, any corner
│                         │
│      WALL / SURFACE     │
│                         │
│             [TAG ID=?]  │  ← the other tag, any corner
└─────────────────────────┘
```

- Tag family: **tag25h9**
- Tag IDs: **0 and 1** — placement order does not matter, script matches by ID
- Both tags must occupy the **same physical positions** in the before and after shots
...
- Get tag PNG files from:
  https://github.com/AprilRobotics/apriltag-imgs/tree/master/tag25h9
  → `tag25h9_00000.png` (ID 0) and `tag25h9_00001.png` (ID 1)
...
```bash
# First time — build the image (takes ~1 min)
docker compose build

# Every time — drop images in input/, then:
docker compose run --rm crack-detector

# Output appears in output/diff-*.png and output/diff_debug-*.png
```

No Python, no pip, no virtualenv needed on the host machine — just Docker.
